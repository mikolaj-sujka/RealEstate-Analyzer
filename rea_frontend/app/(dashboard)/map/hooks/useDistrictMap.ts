"use client";
import { MapRow, OtodomDistrictStat } from "@/services/api/models/types/otodom-listings";
import { OtodomListingsService } from "@/services/api/Otodom";
import { useCallback, useEffect, useMemo, useState } from "react";

function toMapRow(row: OtodomDistrictStat): MapRow {
    return {
        averagePrice: row.averagePricePerSqm,
        properties: row.totalBuildingOffers,
        label: row.district,
        averageFlatSize: row.averageFlatSize,
        averageBuildingBuiltYear: row.averageBuildingBuiltYear,
    };
}
...
export const useDistrictMap = (initialCity: string = "Warszawa") => {
    ...
    const [propertiesRange, setPropertiesRange] = useState<[number, number]>([0, 1]);
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const data = await OtodomListingsService.getDistrictStats(selectedCity, controller.signal);
            setRaw(data ?? []);
        } catch (e: any) {
    ...
    const mapData = useMemo(() => raw.map(toMapRow), [raw]);
    ...
    }, [mapData, priceConfig.min, priceConfig.max, propertiesConfig.min, propertiesConfig.max]);
    ...
    const filterConfig = [
        ...
        },
        {
            ...
            step: Math.max(1, Math.round((propertiesConfig.max - propertiesConfig.min) / 100)),
