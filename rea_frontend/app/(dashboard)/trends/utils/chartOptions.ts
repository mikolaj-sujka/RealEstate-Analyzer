...
import { advancedMetricsData, cityTrendData, correlationData, investmentMetrics, riskAnalysisData } from "../models"

type CityTrendData = (typeof cityTrendData)[keyof typeof cityTrendData]
type AdvancedMetricsData = (typeof advancedMetricsData)[keyof typeof advancedMetricsData]
type InvestmentMetricsData = (typeof investmentMetrics)[keyof typeof investmentMetrics]
type RiskAnalysisData = (typeof riskAnalysisData)[keyof typeof riskAnalysisData]

export const getBasicTrendChartOption = (data: CityTrendData): EChartsOption => ({
    ...
    tooltip: {
        ...
        formatter: (params: any) => {
            ...
            params.forEach((p: any) => {
                tooltip += `${p.marker} ${p.seriesName}: <strong>${p.value.toLocaleString("pl-PL")}</strong><br/>`
            })
            return tooltip
        },
    },
...
export const getAdvancedMetricsChartOption = (data: AdvancedMetricsData): EChartsOption => ({
    backgroundColor: "transparent",
    tooltip: {
        trigger: "axis",
        backgroundColor: "rgba(255, 255, 255, 0.95)",
            ...
            params.forEach((p: any) => {
                const unit = p.seriesName.includes("Czas")
                    ...
                    : p.seriesName.includes("ROI")
                        ? "%"
                        : p.seriesName.includes("Zadłużenie")
                            ? "%"
    ...
    legend: {
        data: ["Wskaźnik Cena/Dochód", "Czas Sprzedaży", "Pustostany", "ROI", "Zadłużenie", "Aktywność Dev."],
        top: 10,
        textStyle: { color: "#6b7280" },
        type: "scroll",
    },
    ...
    series: [
        ...
        },
        {
            ...
            type: "line",
            yAxisIndex: 0,
            data: data.map((item) => item.vacancyRate),
            smooth: true,
            lineStyle: { color: "#8b5cf6", width: 2 },
            itemStyle: { color: "#8b5cf6" },
        },
        ...
        },
        {
            ...
            type: "line",
            yAxisIndex: 1,
            data: data.map((item) => item.mortgageDebt),
            smooth: true,
...
export const getInvestmentChartOption = (data: InvestmentMetricsData): EChartsOption => ({
    backgroundColor: "transparent",
    tooltip: {
        trigger: "axis",
        backgroundColor: "rgba(255, 255, 255, 0.95)",
    ...
    legend: {
        data: ["Płynność", "Sezonowość", "Spekulacja", "Dojrzałość"],
        top: 10,
        textStyle: { color: "#6b7280" },
    },
    radar: {
        indicator: [
            { name: "Płynność", max: 100 },
            { name: "Sezonowość", max: 100 },
            { name: "Spekulacja", max: 100 },
            { name: "Dojrzałość", max: 100 },
        ],
        center: ["50%", "55%"],
        radius: "70%",
    ...
    series: data.map((item, index) => ({
        ...
        data: [
            {
                value: [item.liquidityIndex, item.seasonalityIndex, item.speculationIndex, item.maturityIndex],
                name: item.month,
                lineStyle: { color: `hsl(${index * 60}, 70%, 50%)` },
                itemStyle: { color: `hsl(${index * 60}, 70%, 50%)` },
                areaStyle: { color: `hsl(${index * 60}, 70%, 50%, 0.1)` },
            },
        ],
...
export const getRiskAnalysisChartOption = (data: RiskAnalysisData): EChartsOption => ({
    backgroundColor: "transparent",
    series: [
        {
            type: "gauge",
...
export const getCorrelationChartOption = (): EChartsOption => ({
    ...
    tooltip: {
        ...
        Zmiana: <strong style="color: ${data.change.startsWith("+") ? "#10b981" : "#ef4444"}">${data.change
                }</strong>
      `
        },
    },
